...
def sim_dem(dt, t_end, element_width, total_width, freq=2e3, amp=1e-5, b=0, k=1e6, dm=1e-9, g=0):
    ...
    # predefine support positions to save calculation time, though this will be slower if the simulation is very long
    support = amp * np.sin(2 * np.pi * freq * times)
    M[..., 0] = (np.random.random(M[..., 0].shape) - 0.5) * amp / 10  # todo remove this?
    M[xs, ys, 0] = amp * np.sin(2 * np.pi * freq * (0 - 2 * dt))
    M[xs, ys, 1] = amp * np.sin(2 * np.pi * freq * (0 - 1 * dt))
    ...
    with open('outputs/output.txt', 'w') as outputfile, open('outputs/outputbig.txt', 'w') as outputbigfile:
        for i, t in tqdm(enumerate(times), total=len(times)):
            ...
            if b == 0:
                dz_x = np.pad(np.diff(M[..., 1], axis=0), 1)[:, 1:-1]  # pad both then un-pad y
                dz_y = np.pad(np.diff(M[..., 1], axis=1), 1)[1:-1, :]  # pad both then un-pad x
                M[xm, ym, 2] = (dz_x[:-1, :] + dz_y[:, :-1] - (dz_x[1:, :] + dz_y[:, 1:])
                                )[xm, ym] * km * dt2 + (2 * M[xm, ym, 1] - M[xm, ym, 0]) - (g * dt2)

                # no convolution
                # f1 = (np.sqrt((M[xm, ym, 1] - M[xm + 1, ym, 1]) ** 2 + dx2) - dx) * 2 * (
                #         (M[xm, ym, 1] - M[xm + 1, ym, 1] >= 0) - 0.5)
                ...
                dz_x = np.pad(np.diff(M[..., 1], axis=0), 1)[:, 1:-1]
                dz_y = np.pad(np.diff(M[..., 1], axis=1), 1)[1:-1, :]
                v = M[xm, ym, 1] - M[xm, ym, 0]
                M[xm, ym, 2] = ((dz_x[:-1, :] + dz_y[:, :-1] - (dz_x[1:, :] + dz_y[:, 1:])
                                 )[xm, ym] * km - b * v) * dt2 + M[xm, ym, 1] + v - (g * dt2)
            # recording data
            if i % 10 == 0:  # only write every ___ timestep
                a = M[xm, ym, 2]
                ...
                # write step, time, rms pos of membrane, pos of support
            if i % int((t_end / dt) / 1000) == 0:  # write 500 total frames
                for n in range(M.shape[1]):
